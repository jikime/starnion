...
│   ├── timezone/                ← 🕐 세계시간 스킬
│   │   ├── SKILL.md
│   │   └── tools.py              ← get_world_time, convert_timezone
│   │
│   ├── wordcount/               ← ✏️ 글자수 스킬
│   │   ├── SKILL.md
│   │   └── tools.py              ← count_text
...
│   ├── encode/                  ← 🔐 인코딩 스킬
│   │   ├── SKILL.md
│   │   └── tools.py              ← encode_decode
│   │
│   ├── hash/                    ← 🔑 해시 스킬
│   │   ├── SKILL.md
│   │   └── tools.py              ← generate_hash
│   │
│   ├── color/                   ← 🎨 색상변환 스킬
│   │   ├── SKILL.md
│   │   └── tools.py              ← convert_color
│   │
│   ├── horoscope/               ← ♈ 운세 스킬
│   │   ├── SKILL.md
│   │   └── tools.py              ← get_horoscope
│   │
│   └── ip/                      ← 📡 IP 조회 스킬
│       ├── SKILL.md
│       └── tools.py              ← lookup_ip
...
| 27 | `timezone` | 🕐 세계시간 | get_world_time, convert_timezone | ON | 1 |
| 28 | `wordcount` | ✏️ 글자수 | count_text | ON | 1 |
| 29 | `encode` | 🔐 인코딩 | encode_decode | ON | 1 |
| 30 | `hash` | 🔑 해시 | generate_hash | ON | 1 |
| 31 | `color` | 🎨 색상변환 | convert_color | ON | 1 |
| 32 | `horoscope` | ♈ 운세 | get_horoscope | ON | 1 |
| 33 | `ip` | 📡 IP 조회 | lookup_ip | ON | 1 |
...
# default 없음 — cross-request 파일 누출 방지
_pending_files: ContextVar[list] = ContextVar("pending_files")

def init_pending_files() -> None:
    """gRPC 핸들러에서 에이전트 호출 전에 반드시 호출.
    LangGraph가 도구를 자식 asyncio Task에서 실행할 수 있으므로,
    부모에서 미리 list를 초기화해야 자식이 같은 객체 참조를 상속받아
    in-place append가 부모에서도 보임."""
    _pending_files.set([])
...
> **ContextVar 주의점**: LangGraph ToolNode가 별도 asyncio Task에서 도구를 실행하면, 자식 Task는 부모 context의 **사본**을 상속합니다. `init_pending_files()`로 부모에서 list를 미리 설정하면, 자식이 같은 list 객체 참조를 상속받아 `append()` 결과가 부모에서도 보입니다.
...
tools = [
    ...
    convert_unit,
    get_world_time, convert_timezone,
    count_text,
    encode_decode,
    generate_hash,
    convert_color,
...
| | DOC (레거시) | olefile best-effort + python-docx fallback | ✅ 구현 |
| | XLS (레거시) | openpyxl fallback + 변환 안내 | ✅ 구현 |
| | PPT (레거시) | python-pptx fallback + 변환 안내 | ✅ 구현 |
| **생성** | PDF | ReportLab | ✅ 구현 |
| | DOCX | python-docx | ✅ 구현 |
| | XLSX | openpyxl | ✅ 구현 |
| | PPTX | python-pptx | ✅ 구현 |
...
순수 Python (zoneinfo + datetime, API/LLM 불필요)

| 기능 | 도구 | 구현 | 상태 |
|------|------|------|------|
| 세계 시간 조회 | get_world_time | zoneinfo + datetime, 20+ 도시 매핑 | ✅ 구현 |
| 시간대 변환 | convert_timezone | zoneinfo 기반 시간 변환 | ✅ 구현 |

**특징:**
- 20+ 주요 도시 한국어/영어 이름 매핑 (서울, 뉴욕, 도쿄, 런던 등)
- IANA 타임존 직접 입력 지원 (예: Asia/Tokyo)
- HH:MM 또는 YYYY-MM-DD HH:MM 시간 형식 지원
- 날짜 변경 자동 감지 ("다음 날", "전날" 표시)
- UTC 오프셋 표시
- 추가 의존성 없음 (Python 3.9+ zoneinfo 표준 라이브러리)
...
| 기능 | 도구 | 구현 | 상태 |
|------|------|------|------|
| 텍스트 분석 | count_text | 글자수/단어수/문장수/줄수/바이트수 | ✅ 구현 |

**특징:**
- 글자수 (공백 포함/제외)
- 단어수 (공백 기준)
- 문장수 (마침표, 물음표, 느낌표, 한국어 문장부호 기준)
- 줄수, 바이트수 (UTF-8)
- 텍스트 길이 제한 100,000자
- 추가 의존성 없음
...
순수 Python (base64, urllib.parse, html, API/LLM 불필요)

| 기능 | 도구 | 구현 | 상태 |
|------|------|------|------|
| 인코딩/디코딩 | encode_decode | Base64, URL, HTML 3가지 형식 | ✅ 구현 |

**특징:**
- Base64: base64.b64encode/b64decode (UTF-8)
- URL: urllib.parse.quote/unquote
- HTML: html.escape/html.unescape
- 텍스트 길이 제한 50,000자
- 유효하지 않은 Base64 디코딩 에러 처리
- 추가 의존성 없음

### `hash` — 해시

순수 Python (hashlib, API/LLM 불필요)

| 기능 | 도구 | 구현 | 상태 |
|------|------|------|------|
| 해시 생성 | generate_hash | MD5, SHA1, SHA256, SHA512 | ✅ 구현 |

**특징:**
- 지원 알고리즘: md5, sha1, sha256 (기본값), sha512
- hashlib.new() 기반 (UTF-8 인코딩)
- 텍스트 길이 제한 100,000자
- 한글 텍스트 지원
- 추가 의존성 없음
...
순수 Python (colorsys, API/LLM 불필요)

| 기능 | 도구 | 구현 | 상태 |
|------|------|------|------|
| 색상 코드 변환 | convert_color | HEX, RGB, HSL 상호 변환 | ✅ 구현 |

**특징:**
- HEX 입력: #RGB (3자리) 또는 #RRGGBB (6자리), # 생략 가능
- RGB 입력: rgb(R,G,B) 또는 R,G,B
- 이름 입력: CSS 색상 이름 (40+ 영어/한국어)
- colorsys.rgb_to_hls() 기반 HSL 계산
- 색상별 이모지 매핑 (🔴🟠🟡🟢🔵🟣⚫⚪)
- 추가 의존성 없음

### `horoscope` — 운세

API: ohmanda (무료, API 키 불필요, https://ohmanda.com/api/horoscope)

| 기능 | 도구 | 구현 | 상태 |
|------|------|------|------|
| 별자리 운세 | get_horoscope | ohmanda API (httpx) | ✅ 구현 |

**특징:**
- 12궁 별자리 한국어/영어 이름 매핑
- 별자리별 이모지 + 날짜 범위 표시
- httpx follow_redirects=True (301 리다이렉트 처리)
- 타임아웃/HTTP 에러 한국어 메시지
- 추가 의존성 없음 (httpx 기존 사용)

### `ip` — IP 조회

API: ip-api.com (무료, API 키 불필요) + ipify.org (공인 IP 조회)

| 기능 | 도구 | 구현 | 상태 |
|------|------|------|------|
| IP 주소 조회 | lookup_ip | ip-api.com + ipify.org (httpx) | ✅ 구현 |

**특징:**
- IP 주소 지정 시 해당 IP의 위치/ISP 정보 조회
- IP 미지정 시 서버 공인 IP 자동 조회 (ipify.org)
- 반환 정보: 국가, 지역, 도시, 우편번호, 좌표, 시간대, ISP, 조직, AS
- IPv4, IPv6 지원
- API 실패 시 한국어 에러 메시지
- 추가 의존성 없음 (httpx 기존 사용)
...
| `skills/timezone/SKILL.md` + `tools.py` | get_world_time + convert_timezone (zoneinfo, 순수 Python) |
| `skills/wordcount/SKILL.md` + `tools.py` | count_text (순수 Python) |
| `skills/encode/SKILL.md` + `tools.py` | encode_decode (base64/url/html, 순수 Python) |
| `skills/hash/SKILL.md` + `tools.py` | generate_hash (hashlib, 순수 Python) |
| `skills/color/SKILL.md` + `tools.py` | convert_color (colorsys, 순수 Python) |
| `skills/horoscope/SKILL.md` + `tools.py` | get_horoscope (ohmanda API, httpx) |
| `skills/ip/SKILL.md` + `tools.py` | lookup_ip (ip-api.com + ipify.org, httpx) |
...
Phase 2: Documents (문서 확장)                    ✅ 완료
  ├─ document/parser.py 확장 (docx, doc, xlsx, xls, pptx, ppt, hwp, md, txt, csv)
  │   └─ 레거시 포맷 (doc/xls/ppt): olefile best-effort + 최신 라이브러리 fallback
  ├─ document/generator.py (pdf, docx, xlsx, pptx, md, txt)
  ├─ Proto: FILE 타입 + file_data/name/mime
  ├─ Gateway: FILE 타입 처리 (사진/문서/음성/비디오 분기) — unary + stream 양쪽
  ├─ skills/file_context.py: init_pending_files() + ContextVar 자식 Task 공유
...
Phase 11: Tier 3 Skills (시간대/글자수/인코딩/해시/색상/운세) ✅ 완료
  ├─ timezone/tools.py (get_world_time + convert_timezone via zoneinfo, 20+ 도시)
  ├─ wordcount/tools.py (count_text, 글자수/단어수/문장수/줄수/바이트수)
  ├─ encode/tools.py (encode_decode, base64/url/html 인코딩/디코딩)
  ├─ hash/tools.py (generate_hash, md5/sha1/sha256/sha512)
  ├─ color/tools.py (convert_color, HEX/RGB/HSL + 한국어 색상명)
  ├─ horoscope/tools.py (get_horoscope via ohmanda API, 12궁 한/영 매핑)
  ├─ 추가 의존성 없음 (zoneinfo/colorsys/hashlib 표준 라이브러리, httpx 기존 사용)
  └─ 단위 테스트 424 → 501개 확장

Phase 12: IP Lookup Skill                                       ✅ 완료
  ├─ ip/tools.py (lookup_ip via ip-api.com + ipify.org, httpx)
  ├─ 추가 의존성 없음 (httpx 기존 사용)
  └─ 단위 테스트 501 → 510개 확장

Phase 13: Document Pipeline Hardening                             ✅ 완료
  ├─ 레거시 파서 (doc/xls/ppt): olefile best-effort + 최신 라이브러리 fallback
  │   ├─ extract_text_from_doc: python-docx fallback → olefile FIB/piece-table 파싱
  │   ├─ extract_text_from_xls: openpyxl fallback → 변환 안내
  │   └─ extract_text_from_ppt: python-pptx fallback → 변환 안내
  ├─ PPTX 생성 추가: generate_pptx (python-pptx, 제목 슬라이드 + 내용 슬라이드)
  ├─ 파일 전달 파이프라인 수정:
  │   ├─ file_context.py: init_pending_files() 추가 (ContextVar 자식 Task 공유)
  │   ├─ server.py Chat(): init_pending_files() + pop_pending_files() 추가
  │   ├─ server.py ChatStream(): init_pending_files() 추가
...
- **파일 생성 (PDF)**: "보고서 PDF로 만들어줘" → Telegram에서 PDF 파일 수신 확인
- **파일 생성 (PPT)**: "발표자료 만들어줘" → Telegram에서 PPTX 파일 수신 확인
- **파일 생성 (Unary)**: 스트리밍 실패 시 unary 폴백에서도 파일 수신 확인
...
- **세계시간**: "뉴욕 지금 몇시야?" → 현재 시간 반환
- **시간대 변환**: "서울 오후 3시면 런던 몇시야?" → 시간 변환 결과
- **글자수**: "이 텍스트 몇 글자야?" → 글자수/단어수/문장수/바이트수
- **인코딩**: "Hello World base64로 인코딩해줘" → SGVsbG8gV29ybGQ=
...
| `test_timezone_tools.py` | get_world_time, convert_timezone, 도시 매핑, 검증 | 16 |
| `test_wordcount_tools.py` | count_text, 한글/영문/혼합 텍스트, 검증 | 11 |
| `test_encode_tools.py` | encode_decode, base64/url/html 인코딩, 라운드트립 | 13 |
| `test_hash_tools.py` | generate_hash, md5/sha1/sha256/sha512, 한글 | 10 |
...
# SKILL.md 로딩: 30개 문서
# 파일 컨텍스트 파이프라인: OK
# 문서 파서/생성기: OK (6종 생성: PDF/DOCX/XLSX/PPTX/MD/TXT, 10종 파싱)
# Go 빌드 + vet: OK
```
...
**상태**: ✅ 구현 완료 (Phase 1-12 + Tool Status)
**날짜**: 2026-03-02
