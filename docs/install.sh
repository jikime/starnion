...
# Install gateway server binary, agent, web and scripts to ~/.starnion/
...
if [[ -d "$TMPDIR_WORK/agent" ]]; then
  ...
  # Install production node_modules on the server (not bundled in tarball)
  if command -v pnpm &>/dev/null; then
    info "Agent 의존성 설치 중 (pnpm install --prod)..."
    (cd "${STARNION_HOME}/agent" && pnpm install --frozen-lockfile --prod --ignore-scripts) \
      && ok "Agent 의존성 설치 완료" \
      || warn "Agent 의존성 설치 실패 — 수동으로 실행하세요: cd ~/.starnion/agent && pnpm install --prod"
  elif command -v npm &>/dev/null; then
    info "Agent 의존성 설치 중 (npm install --omit=dev)..."
    (cd "${STARNION_HOME}/agent" && npm install --omit=dev --ignore-scripts) \
      && ok "Agent 의존성 설치 완료" \
      || warn "Agent 의존성 설치 실패 — 수동으로 실행하세요: cd ~/.starnion/agent && npm install --omit=dev"
  else
    warn "pnpm 또는 npm이 설치되어 있지 않습니다. Agent 실행 전 수동으로 의존성을 설치하세요:"
    warn "  npm install -g pnpm"
    warn "  cd ~/.starnion/agent && pnpm install --frozen-lockfile --prod"
  fi
fi

# Create StarNion Python virtual environment and install skill dependencies
VENV_DIR="${STARNION_HOME}/venv"
REQUIREMENTS="${STARNION_HOME}/agent/skills/requirements.txt"
info "Python 가상환경 설정 중 (${VENV_DIR})..."
if command -v uv &>/dev/null; then
  uv venv "$VENV_DIR" --quiet 2>/dev/null \
    && { [[ -f "$REQUIREMENTS" ]] && uv pip install -r "$REQUIREMENTS" --python "$VENV_DIR/bin/python" --quiet 2>/dev/null || true; } \
    && ok "Python 가상환경 + 의존성 설치 완료 (uv)" \
    || warn "Python 가상환경 설치 실패 — 수동 실행: uv venv ~/.starnion/venv && uv pip install -r ~/.starnion/agent/skills/requirements.txt -p ~/.starnion/venv/bin/python"
elif command -v python3 &>/dev/null; then
  python3 -m venv "$VENV_DIR" 2>/dev/null \
    && { [[ -f "$REQUIREMENTS" ]] && "$VENV_DIR/bin/pip" install -r "$REQUIREMENTS" --quiet 2>/dev/null || true; } \
    && ok "Python 가상환경 + 의존성 설치 완료 (python3 venv)" \
    || warn "Python 가상환경 설치 실패 — 수동 실행: python3 -m venv ~/.starnion/venv && ~/.starnion/venv/bin/pip install -r ~/.starnion/agent/skills/requirements.txt"
else
  warn "uv 또는 python3이 없습니다. AI 스킬 사용 전 수동으로 설치하세요:"
  warn "  curl -LsSf https://astral.sh/uv/install.sh | sh"
  warn "  uv venv ~/.starnion/venv"
  warn "  uv pip install -r ~/.starnion/agent/skills/requirements.txt -p ~/.starnion/venv/bin/python"
fi

if [[ -d "$TMPDIR_WORK/web" ]]; then
  rm -rf "${STARNION_HOME}/web"
  mv "$TMPDIR_WORK/web" "${STARNION_HOME}/web"
fi

# Install proto/ to ~/.starnion/proto/ (gRPC definitions needed by agent)
if [[ -d "$TMPDIR_WORK/proto" ]]; then
  rm -rf "${STARNION_HOME}/proto"
  mv "$TMPDIR_WORK/proto" "${STARNION_HOME}/proto"
fi

# Install scripts/ to ~/.starnion/scripts/
if [[ -d "$TMPDIR_WORK/scripts" ]]; then
  rm -rf "${STARNION_HOME}/scripts"
  mv "$TMPDIR_WORK/scripts" "${STARNION_HOME}/scripts"
fi
