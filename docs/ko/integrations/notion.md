...
Starnion과 Notion을 연결하면 AI 에이전트가 자연어로 Notion 페이지를 검색하고, 새 페이지를 생성하고, 내용을 읽거나 추가할 수 있습니다. 회의록 작성, 아이디어 기록, 지식 베이스 검색 등에 활용할 수 있습니다.
...
**Q: Notion 데이터베이스 레코드를 추가할 수 있나요?**
A: 현재는 페이지 생성, 내용 읽기, 블록 추가를 지원합니다. 데이터베이스의 속성(프로퍼티)을 직접 조작하는 기능은 지원하지 않습니다.
...
A: 현재는 페이지 끝에 새 블록을 추가하는 방식만 지원합니다. 기존 블록 내용의 수정은 지원하지 않습니다.
