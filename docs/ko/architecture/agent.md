...
Agent는 Starnion의 AI 두뇌입니다. TypeScript/Node.js로 작성되었으며 Anthropic AI SDK v6을 기반으로 동작합니다. Gateway로부터 gRPC 요청을 받아 AI 추론, 스킬 실행을 수행하고 최종 응답을 반환합니다.

**핵심 역할:**
- 사용자 메시지를 분석하여 의도 파악
- 적절한 스킬(Tool)을 선택하고 실행 (diary, finance, goals, image)
- Anthropic Claude 모델로 응답 생성
...
Agent는 Anthropic AI SDK v6의 Tool Use 패턴을 사용합니다.
...
└─────────────────────────────────────────┘
      │ SSE 스트리밍 응답
      ▼
   gRPC 스트리밍 → Gateway → 클라이언트
```

### 동작 흐름 요약

1. **입력 수신**: Gateway로부터 gRPC 요청 수신 (사용자 메시지 + 세션 ID)
2. **컨텍스트 로딩**: 대화 이력 및 사용자 설정 로드
3. **LLM 추론**: 시스템 프롬프트 + 대화 이력을 Claude 모델에 전달
4. **스킬 실행**: 모델이 필요한 스킬을 선택하면 해당 함수 실행
5. **응답 스트리밍**: SSE 형식으로 gRPC 스트림을 통해 실시간 전송
