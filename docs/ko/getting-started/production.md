...
| Node.js | 18.12+ — `node server.js` 실행을 위해 필요 (pnpm 불필요) |
...
`starnion-ui.service`는 `node server.js`로 UI를 실행합니다 (Next.js standalone 빌드).
**`starnion update`를 실행하면 node 경로가 자동으로 패치됩니다.** 수동 초기 설치 시에만 아래를 따릅니다.
...
> **💡 자동 패치**: `starnion update` 실행 시 `~/.starnion/scripts/` 의 최신 서비스 파일을 `/etc/systemd/system/`에 복사하고 실제 node 경로로 자동 패치합니다.
...
Next.js UI는 **릴리스 tarball에 standalone 빌드로 포함**됩니다. `starnion setup` 또는 `starnion update` 실행 시 `~/.starnion/ui/`에 자동으로 설치되므로 **서버에서 별도 빌드가 필요하지 않습니다.**

```bash
# UI 파일 설치 확인
ls ~/.starnion/ui/server.js    # 존재해야 합니다
```

설치되어 있지 않다면:

```bash
starnion update --force
```
...
| `status=127` / `node: not found` | node 경로 미설정 | 3단계 참고하여 `ExecStart` node 경로 수정 |
| `Cannot find module '/root/.starnion/ui/server.js'` | UI standalone 파일 미설치 | `starnion update --force` 를 두 번 실행 |
