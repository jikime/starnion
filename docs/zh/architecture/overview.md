...
└───────────────┘   │       │                │            │
         ...
         ┌────────────────────────────────────────────────┐  │
         │              LLM 提供商                        │──┘
         │  Gemini / OpenAI / Claude / Ollama（本地）     │
         └────────────────────────────────────────────────┘
```
...
### 1. UI（Next.js）— 端口 3000

Web 前端。这是用户在浏览器中直接交互的界面。

- **聊天界面：** 实时流式响应、文件附件、对话历史
- **仪表板：** 消费摘要、目标状态、近期活动
- **设置：** 技能管理、频道（Telegram）配置、提供商配置
- **搜索栏：** 通过顶部搜索栏进行快速本地搜索
...
- **gRPC 客户端：** 与 Python Agent 通信
- **调度器：** 运行提醒和报告等定期任务
- **MinIO 集成：** 将上传的文件存储到对象存储

Go 的高并发能力确保即使同时有多个用户连接也能稳定运行。

### 3. Agent（Python）— 端口 50051（gRPC）

AI 大脑。基于 LangGraph 的智能体分析消息并执行技能。

- **LangGraph Agent：** 消息路由、技能选择、响应生成
- **技能系统：** 34 个独立技能模块（天气、财务、日记等）
- **四层 RAG 记忆：** 通过向量搜索引用历史记录
- **多 LLM 支持：** 可在 Gemini、OpenAI、Claude、Ollama 等多个提供商之间切换
...
存储的数据：对话历史、消费记录、日记条目、备忘录、目标、倒计时、文档索引、嵌入向量、频道设置、技能设置
...
Gateway（Go）与 Agent（Python）通过 gRPC 通信。
...
Web UI 和 Telegram 连接到**同一个** Python Agent。
...
Agent 支持多个 LLM 提供商，用户或管理员可以在设置中切换。

| 提供商 | 示例模型 | 备注 |
|--------|---------|------|
| Google Gemini | gemini-2.0-flash | 快速响应，多模态 |
| OpenAI | gpt-4o、gpt-4o-mini | 高质量响应 |
| Anthropic Claude | claude-3-5-sonnet | 长上下文处理 |
| Ollama | llama3、mistral | 完全本地（无需网络） |
...
Web UI 登录基于 JWT（JSON Web Token）。
