...
외부 메시징 플랫폼을 Nion에 연결하는 시스템. 사용자별로 독립된 봇 인스턴스를 운영하며, DM·그룹 정책을 각자 설정할 수 있다.
...
봇에게 `/link` 명령어를 보내면 임시 코드(예: `NION-7A4B2F`)를 발급한다.
이 코드를 채널 UI → 채널 설정 → 계정 연결에 입력하면 Telegram 계정과 Nion 계정이 연결된다.
