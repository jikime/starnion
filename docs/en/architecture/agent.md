...
The Agent is the AI brain of Starnion. Written in Python, it operates on a LangGraph ReAct architecture. It receives gRPC requests from the Gateway, performs AI reasoning and skill execution and memory retrieval, and returns the final response.

**Core roles:**
- Analyze user messages to understand intent
- Select and execute the appropriate skill (Tool)
- Search the 4-Layer memory system for relevant information
- Multi-LLM routing (select model based on user settings)
