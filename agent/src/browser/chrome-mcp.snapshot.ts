import type { RoleRefMap, RoleSnapshotOptions, SnapshotAriaNode, SnapshotStats } from "./types.js";
...
function createDuplicateTracker(): DuplicateTracker {
  return { counts: new Map(), keysByRef: new Map(), duplicates: new Set() };
}
...
function getRoleSnapshotStats(snapshot: string, refs: RoleRefMap): SnapshotStats {
  ...
  return { lines, chars, refs: refCount, interactive };
}
...
export function buildAiSnapshotFromChromeMcpSnapshot(params: {
  ...
  const visit = (node: ChromeMcpSnapshotNode, depth: number) => {
    ...
    if (shouldIncludeNode({ role, name, options: params.options })) {
