...
keywords: ["웹사이트", "URL", "열어줘", "website", "browse", "navigate", "ウェブ", "网页", "打开网站"]
---
...
URL 하나를 열고 스크린샷이 필요할 때는 **이 툴 하나로 시작**하세요.
내부에서 navigate → networkidle 대기 → 렌더링 대기 → 전체 페이지 캡처를 자동으로 처리합니다.
**스크린샷 후에도 세션이 유지**되므로 이어서 클릭·입력 등 인터랙션이 가능합니다.
...
1. `browser_open_screenshot` 또는 `browser_navigate` — URL로 이동 및 초기 스크린샷
2. `browser_snapshot` — 페이지 구조 파악 (클릭 가능한 요소 확인)
3. `browser_click` / `browser_hover` / `browser_type` — 상호작용
4. `browser_screenshot` — 결과 확인 (이미지로 전송됨)
5. `browser_close` — 작업 완료 후 브라우저 종료

**드롭다운 메뉴**는 `browser_hover`로 열고 → `browser_click`으로 항목을 선택하세요.
