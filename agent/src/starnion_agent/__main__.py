...
from starnion_agent.db.pool import close_pool, get_pool, init_pool
from starnion_agent.graph.agent import close_checkpointer, create_agent
from starnion_agent.grpc.server import serve
from starnion_agent.log_buffer import install as install_log_buffer, set_embed_search_callback, set_index_callback, start_http_server
from starnion_agent.skills.registry import register_skills
...
async def _index_document(
    ...
    The gateway has already inserted the row into documents; this
    ...
    try:
        ...
        # Mark the document as indexed in documents if the column exists.
        try:
...
async def _embed_search(user_id: str, search_id: int) -> None:
    """Embed a saved web search result and store the vector in searches.
    ...
    try:
        ...
        async with pool.connection() as conn:
            async with conn.cursor() as cur:
                await cur.execute(
                    "SELECT query, result FROM searches WHERE id = %s AND user_id = %s",
                    (search_id, user_id),
                )
