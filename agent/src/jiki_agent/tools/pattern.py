...
async def analyze_patterns(user_id: str) -> str:
    ...
    Returns:
        A summary string describing the analysis result (for logging).
    """
    pool = get_pool()
...
async def generate_pattern_insight(user_id: str) -> str:
    ...
    Reads patterns from knowledge_base and combines with current context.

    Returns:
        A natural-language notification message, or empty string if no patterns.
    """
    pool = get_pool()
