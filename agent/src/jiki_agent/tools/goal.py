"""Goal management tools and report functions for Level 3 autonomous agent.
...
async def evaluate_goals(user_id: str) -> str:
    ...
    Called daily via gRPC GenerateReport(report_type="goal_evaluate").
    Results are stored in knowledge_base but NOT sent to the user.
...
async def generate_goal_status(user_id: str) -> str:
    ...
    Called via gRPC GenerateReport(report_type="goal_status").
    Returns a natural-language notification message, or empty string.
    """
    pool = get_pool()
    entries = await knowledge_repo.get_by_key_prefix(pool, user_id, GOAL_KEY_PREFIX)
...
def _build_evaluation_data(
    ...
    days_left = last_day - day_of_month
...
def _build_status_prompt(
    ...
    # Goal summaries with evaluations.
