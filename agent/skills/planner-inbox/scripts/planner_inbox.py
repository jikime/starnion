...
def cmd_search(args):
    kw = esc(args.keyword)
    rows = psql(f"SELECT id, title, priority FROM planner_tasks WHERE user_id='{args.user_id}' AND is_inbox=TRUE AND title ILIKE '%{kw}%' ORDER BY sort_order LIMIT 10;")
...
def cmd_add(args):
    ...
    psql(f"INSERT INTO planner_tasks (user_id, title, is_inbox, task_date, sort_order) VALUES ('{args.user_id}', '{title}', TRUE, '{today()}', (SELECT COALESCE(MAX(sort_order),0)+1 FROM planner_tasks WHERE user_id='{args.user_id}' AND is_inbox=TRUE));")
    print(f"📥 임시보관 추가: {args.title}")

def cmd_list(args):
    rows = psql(f"SELECT id, title, priority FROM planner_tasks WHERE user_id='{args.user_id}' AND is_inbox=TRUE ORDER BY sort_order;")
...
def cmd_promote(args):
    ...
    result = psql(f"UPDATE planner_tasks SET is_inbox=FALSE, priority='{pri}', task_date='{d}', updated_at=NOW() WHERE id={args.id} AND user_id='{args.user_id}' AND is_inbox=TRUE RETURNING title;")
    if not result: print("❌ 해당 항목을 찾을 수 없습니다."); return
    print(f"✅ 업무로 이동: {result.strip()} → {pri} ({d})")

def cmd_delete(args):
    result = psql(f"DELETE FROM planner_tasks WHERE id={args.id} AND user_id='{args.user_id}' AND is_inbox=TRUE RETURNING title;")
    if not result: print("❌ 해당 항목을 찾을 수 없습니다."); return
