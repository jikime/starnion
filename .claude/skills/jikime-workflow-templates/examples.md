...
**Input**:
```python
from jikime_workflow_templates import load_template
...
**Scenario**: GitHub 이슈를 통한 버그 리포트 생성

**Input**:
```bash
# 피드백 이슈 생성
gh issue create --title "로그인 시 JWT 토큰이 만료되어도 갱신되지 않음" --template bug_report.md
```
...
---
Generated by: JikiME-ADK Feedback System
...
**Scenario**: JikiME-ADK 업데이트 후 사용자 커스터마이징을 보존하며 템플릿 병합
...
```json
{
  ...
  "user_customizations_preserved": [
    {
      "file": ".jikime/config/config.yaml",
      "preserved_fields": [
        "user.name",
  ...
  "conflicts_resolved": [
    {
      "file": ".jikime/config/sections/quality.yaml",
      "resolution": "Merged both changes",
      "details": "User's custom thresholds + new quality metrics"
    }
  ],
  "manual_review_required": [],
  "restoration_point": ".jikime/backups/pre-merge-2025-12-06/"
}
```
...
```json
// .jikime/config/template-history.json
{
...
```bash
# 잘못된 예시 - 템플릿 직접 수정
vim .claude/skills/jikime-workflow-templates/modules/code-templates.md
# → JikiME-ADK 업데이트 시 충돌
# → 팀원 간 불일치
```
...
# → .jikime/config/template-overrides.json에 저장
# → 템플릿 업데이트와 독립적
```
...
```
프로젝트 초기화:
/jikime:0-project → 코드 템플릿 선택 → 스캐폴딩 → 버전 추적

피드백 제출:
GitHub Issue → 자동 분류 → 템플릿 적용 → GitHub 이슈 생성
