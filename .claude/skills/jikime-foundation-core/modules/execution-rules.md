...
Purpose: Security policies, execution constraints, and Git workflow strategies governing JikiME-ADK agent behavior.
...
2. Running `/jikime:2-run`: DDD commits + auto push
3. Running `/jikime:3-sync`: Doc commits + suggest PR creation (user choice)
...
2. Running `/jikime:2-run`: DDD commits + auto push (to feature branch)
3. Running `/jikime:3-sync`: Doc commits + prepare PR
...
Execute `/clear` after `/jikime:1-plan` completion in ALL modes.
...
1. Immediately after `/jikime:1-plan` (mandatory)
...
- Create documentation in `.jikime/` directory
...
This comprehensive set of execution rules ensures that JikiME-ADK operates securely, efficiently, and in compliance with industry standards while maintaining high quality output.
