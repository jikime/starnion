...
1. Immediately after /jikime:1-plan (saves 45-50K)
...
| Phase 2: DDD | 180K | Implementation + tests | RED (40K) + GREEN (80K) + REFACTOR (60K) |
...
def restore_minimal_context(summary: dict):
 ...
 load_file(f".jikime/specs/{summary['current_spec']}/spec.md")
...
class SelectiveFileLoader:
 ...
 f".jikime/specs/{context['spec_id']}/spec.md"
 ]
 ...
 files = [f".jikime/specs/{context['spec_id']}/spec.md"]
...
def load_history():
 load_file(".jikime/conversation_history.json") # 500K+ tokens
```
...
class ContextOptimizer:
 ...
 "frontend": ["spec_id", "api_endpoints", "ui_components"],
 "security-auditor": ["spec_id", "threat_model", "dependencies"],
 "test-guide": ["spec_id", "code_structure", "test_strategy"],
...
backend_context = optimizer.optimize_context(large_context, "backend")
...
# Execute /clear here
monitor.record_usage("ddd", "red_phase", 40_000)
monitor.record_usage("ddd", "green_phase", 80_000)
monitor.record_usage("ddd", "refactor_phase", 60_000)
...
Skills:
- jikime-foundation-core - Delegation patterns and progressive disclosure
- jikime-foundation-context - Context persistence

Commands:
- /clear - Context reset (mandatory after /jikime:1-plan)
- /context - Check current token usage
- /jikime:1-plan - SPEC generation (30K budget)
- /jikime:2-run - DDD implementation (180K budget)
- /jikime:3-sync - Documentation (40K budget)

Memory:
- Skill("jikime-foundation-core") modules/token-optimization.md - Optimization strategies
- @.jikime/config/config.json - Budget configuration
