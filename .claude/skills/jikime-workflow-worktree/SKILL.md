...
# JikiME Worktree Management

Git worktree management system for parallel SPEC development with isolated workspaces, automatic registration, and seamless JikiME-ADK integration.
...
What is JikiME Worktree Management?
...
- Seamless Integration: Works with /jikime:1-plan, /jikime:2-run, /jikime:3-sync workflow
...
5. Integration Layer - JikiME-ADK workflow integration
...
The worktree system creates a dedicated directory structure inside the project's .jikime directory. At the worktree root ({repo}/.jikime/worktrees/{ProjectName}/), you will find the central registry JSON file and individual directories for each SPEC. Each SPEC directory contains a .git file for worktree metadata and a complete copy of all project files.
...
During the Plan Phase using /jikime:1-plan, the SPEC is created and the worktree new command sets up automatic worktree isolation.
...
During the Sync Phase using /jikime:3-sync, the worktree sync command ensures clean integration with conflict resolution support.
...
First, create a worktree for SPEC-001 with a description like "User Authentication" and switch to that directory. Then run /jikime:2-run SPEC-001 to develop in isolation. Next, navigate back to the main repository and create another worktree for SPEC-002 with description "Payment Integration". Switch to that worktree and run /jikime:2-run SPEC-002 for parallel development. When needed, switch between worktrees and continue development. Finally, sync both worktrees when ready for integration.

Detailed Reference: Refer to Parallel Development Module at modules/parallel-development.md

---

### 4. Integration Patterns - JikiME-ADK Workflow Integration

Purpose: Seamless integration with JikiME-ADK Plan-Run-Sync workflow.

Integration Points:

During Plan Phase Integration with /jikime:1-plan, after SPEC creation, create the worktree using the new command with the SPEC ID. The output provides guidance for switching to the worktree using either the switch command or the shell eval pattern with the go command.

During Development Phase with /jikime:2-run, worktree isolation provides a clean development environment with independent Git state preventing conflicts and automatic registry tracking.

During Sync Phase with /jikime:3-sync, before PR creation run the sync command for the SPEC. After PR merge, run the clean command with the merged-only flag to remove completed worktrees.
...
The JikiME configuration supports worktree settings including auto_create for automatic worktree creation, auto_sync for automatic synchronization, cleanup_merged for automatic cleanup of merged branches, and worktree_root for specifying the worktree directory location with project name substitution.
...
For new SPEC development, use the worktree isolation pattern with auto-setup. The primary approach is worktree isolation and the supporting pattern is integration with /jikime:1-plan.
...
- Integration Patterns: Refer to modules/integration-patterns.md for JikiME-ADK integration
