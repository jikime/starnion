...
export interface ToolEvent {
  tool: string
  input?: string   // input_json from tool_use
  result?: string  // result from tool_result
  isError?: boolean
...
export interface ChatMessage {
  ...
  streaming: boolean
  timestamp: Date
  // metadata from the agent (set on assistant messages after streaming completes)
  botName?: string
  modelId?: string
  inputTokens?: number
  outputTokens?: number
...
function segmentsFromUIMessage(m: UIMessage): Segment[] {
  ...
  for (const part of m.parts) {
    if (part.type === "text") {
      ...
      const inv = (part as unknown as { toolInvocation: { toolName: string; state: string } }).toolInvocation
      segs.push({
        kind: "tool",
        name: inv.toolName,
...
export function useChat(activeThreadId: string | null) {
  ...
  useEffect(() => {
    ...
    fetch(`/api/conversations/${activeThreadId}/messages?limit=30`)
      ...
      .then((data: { messages: Array<{ id: string; role: string; content: string; created_at: string; attachments?: FileAttachment[]; bot_name?: string; model_used?: string }>; has_more: boolean; next_cursor: string | null }) => {
  ...
  const loadMore = useCallback(async () => {
    ...
    try {
      ...
      if (!res.ok) return
      const data: { messages: Array<{ id: string; role: string; content: string; created_at: string; attachments?: FileAttachment[]; bot_name?: string; model_used?: string }>; has_more: boolean; next_cursor: string | null } = await res.json()
      if (!mountedRef.current) return
