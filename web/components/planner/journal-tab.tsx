...
function DiaryPanel() {
  const t = useTranslations("planner.journal")
  ...
  return (
    <div className="flex flex-col h-full overflow-y-auto px-6 py-4 space-y-5">
      ...
      <div className="space-y-2">
        <p className="text-xs uppercase tracking-widest text-muted-foreground font-medium">{t("todayCondition")}</p>
          ...
          {MOOD_KEYS.map((key) => (
            <button
              ...
              className={cn(
                ...
                mood === key ? "border-transparent" : "border-border bg-muted/30 text-muted-foreground hover:bg-accent/30"
              )}
              style={mood === key
      ...
      <div className="space-y-1.5">
        <p className="text-xs uppercase tracking-widest text-muted-foreground font-medium">{t("todayOneLiner")}</p>
        <Input
          value={oneLiner}
      ...
      {/* Reflection prompts */}
      <div className="space-y-2">
        <p className="text-xs uppercase tracking-widest text-muted-foreground font-medium">{t("reflectionQuestions")}</p>
      ...
      <div className="space-y-1.5 flex-1">
        <p className="text-xs uppercase tracking-widest text-muted-foreground font-medium">{t("freeRecord")}</p>
        <Textarea
          value={fullNote}
...
function InboxPanel() {
  const t = useTranslations("planner.journal")
  const tInbox = useTranslations("planner.inbox")
    ...
    <div className="flex flex-col h-full overflow-hidden">
      <div className="flex items-center justify-between px-6 py-3 border-b border-border shrink-0">
        <div className="flex items-center gap-2">
          ...
          <span className="text-sm font-semibold text-foreground">{tInbox("title")}</span>
          {inboxTasks.length > 0 && (
            <span
      ...
      {adding && (
        <div className="px-6 py-3 border-b border-border bg-accent/20 space-y-2 shrink-0">
          <Input
            ...
            placeholder={tInbox("capturePlaceholder")}
      ...
      <div className="flex-1 overflow-y-auto px-6 py-3 space-y-2">
        {inboxTasks.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-16 gap-2 text-muted-foreground">
            ...
            <p className="text-sm">{tInbox("empty")}</p>
            <p className="text-xs opacity-60">{tInbox("emptyHint")}</p>
          </div>
        ) : (
          inboxTasks.map((task) => {
...
function StatsPanel() {
  ...
  return (
    <div className="flex flex-col h-full overflow-y-auto px-6 py-4 space-y-6">
      ...
      >
        <p className="text-xs uppercase tracking-widest font-medium" style={{ color: "var(--priority-a)" }}>{t("todayCompletion")}</p>
        <div className="flex items-end gap-2">
          <span className="text-4xl font-bold tabular-nums" style={{ color: "var(--priority-a)" }}>
            {score}%
          </span>
          <span className="text-xs text-muted-foreground pb-1">{t("abBasis")}</span>
        </div>
      ...
      <div className="space-y-3">
        <p className="text-xs uppercase tracking-widest text-muted-foreground font-medium">{t("roleDistribution")}</p>
        {roles.map((role) => {
          ...
          return (
            <div key={role.id} className="space-y-1">
              <div className="flex items-center justify-between text-xs">
                ...
                <span className="text-muted-foreground tabular-nums">{t("countPct", { count, pct })}</span>
              </div>
      ...
      <div className="space-y-3">
        <p className="text-xs uppercase tracking-widest text-muted-foreground font-medium">{t("last7days")}</p>
...
export function JournalTab() {
  ...
  const SECTIONS: { id: JournalSection; label: string; icon: React.ReactNode }[] = [
    { id: "diary",  label: t("sectionDiary"),  icon: <BookMarked className="w-3.5 h-3.5" /> },
    { id: "inbox",  label: t("sectionInbox"),  icon: <Inbox className="w-3.5 h-3.5" /> },
    { id: "stats",  label: t("sectionStats"),  icon: <BarChart2 className="w-3.5 h-3.5" /> },
  ]
...
export function ChatSection() {
  ...
  return (
    <div className="flex flex-col flex-1 items-center justify-center gap-4 text-muted-foreground">
      ...
      <p className="text-sm font-medium">{t("chatPreparing")}</p>
    </div>
  )
}
...
export function AnalyticsSection() {
  ...
  return (
    <div className="flex flex-col flex-1 items-center justify-center gap-4 text-muted-foreground">
      ...
      <p className="text-sm font-medium">{t("analyticsPreparing")}</p>
    </div>
  )
}
...
const TYPE_FILTERS: { id: FileType | "search"; labelKey: string; icon: React.ElementType }[] = [
  { id: "all",    labelKey: "filterAll",    icon: Folder },
  { id: "doc",    labelKey: "filterDoc",    icon: FileText },
  { id: "image",  labelKey: "filterImage",  icon: ImageIcon },
  { id: "audio",  labelKey: "filterAudio",  icon: Music },
  { id: "search", labelKey: "filterSearch", icon: Sparkles },
]
...
export function FilesSection() {
  ...
  const tFiles = useTranslations("planner.files")
  ...
  return (
    <div className="flex flex-col flex-1 overflow-hidden bg-background">
      ...
      <div className="flex items-center gap-3 px-5 h-12 border-b border-border shrink-0">
        <span className="text-sm font-bold text-foreground whitespace-nowrap">{t("myFiles")}</span>
        <div className="relative flex-1 max-w-xs">
          <Search className="absolute left-2.5 top-1/2 -translate-y-1/2 w-3.5 h-3.5 text-muted-foreground pointer-events-none" />
          <input
            value={query}
            onChange={e => setQuery(e.target.value)}
            placeholder={t("myFilesSearch")}
        ...
        <div className="flex items-center gap-2 ml-auto">
          ...
          <button className="flex items-center gap-1.5 h-8 px-3 rounded-lg text-xs font-semibold transition-colors"
            ...
            <Plus className="w-3.5 h-3.5" />{t("createNew")}
          </button>
      ...
      <div className="flex items-center gap-2 px-5 py-2.5 border-b border-border shrink-0 overflow-x-auto scrollbar-none">
        {TYPE_FILTERS.map(({ id, labelKey, icon: Icon }) => {
      ...
      <div className="px-5 py-2 shrink-0">
        <span className="text-xs text-muted-foreground">{tFiles("fileCount", { count: filtered.length })}</span>
      </div>
      ...
      <div className="flex-1 overflow-y-auto px-5 pb-5"
        ...
        {filtered.length === 0 ? (
          <div className="flex flex-col items-center justify-center h-48 gap-3 text-muted-foreground">
            ...
            <p className="text-sm">{tFiles("noFiles")}</p>
          </div>
        ) : viewMode === "grid" ? (
                ...
                <div className="relative aspect-[4/3] bg-muted overflow-hidden">
                  ...
                  {menuId === file.id && (
                    <div
                      ...
                      {[
                        { icon: Download, labelKey: "download" },
                        { icon: Pencil,   labelKey: "rename" },
                        { icon: Copy,     labelKey: "copy" },
                      ...
                      <button
                        ...
                        <Trash className="w-3.5 h-3.5" />{tFiles("delete")}
                      </button>
                    </div>
                  )}
                </div>
          ...
          <div className="rounded-xl border border-border bg-card overflow-hidden">
            <div className="grid grid-cols-[2fr_1fr_1fr_1fr_40px] px-4 py-2 border-b border-border bg-muted/40">
              {[tFiles("colName"), tFiles("colType"), tFiles("colSize"), tFiles("colDate"), ""].map((h, i) => (
                  ...
                  {menuId === file.id && (
                    <div
                      ...
                      {[
                        ...
                        <button key={labelKey}
                          className="w-full flex items-center gap-2 px-3 py-1.5 text-xs text-foreground hover:bg-accent/50 transition-colors">
                          <Icon className="w-3.5 h-3.5 text-muted-foreground" />{tFiles(labelKey)}
                        </button>
                      ))}
                      <div className="border-t border-border my-1" />
