...
var jobTranslations = map[string]map[string]jobI18n{
	"en": {
		...
		"connect_activity_ingest":  {Name: "Connect Activity Ingest", Description: "Pulls Gmail and Google Calendar activity into your Connect timeline every night"},
		"connect_score_recompute":  {Name: "Connect Score Recompute", Description: "Recomputes connection scores nightly from recent activity and target cadence"},
		"connect_drift_reminder":   {Name: "Connect Drift Reminder", Description: "Daily summary of the connections you've gone too long without contacting"},
	...
	"ja": {
		...
		"connect_activity_ingest":  {Name: "人脈アクティビティ取り込み", Description: "GmailとGoogleカレンダーから人脈タイムラインへ毎晩アクティビティを取り込みます"},
		"connect_score_recompute":  {Name: "人脈スコア再計算", Description: "最近のアクティビティと目標頻度から毎晩スコアを再計算します"},
		"connect_drift_reminder":   {Name: "人脈ドリフトリマインダー", Description: "連絡が途絶えた人脈を毎日お知らせします"},
	...
	"zh": {
		...
		"connect_activity_ingest":  {Name: "人脉活动同步", Description: "每晚从Gmail和Google日历自动同步活动到人脉时间轴"},
		"connect_score_recompute":  {Name: "人脉评分重算", Description: "每晚根据最近活动和目标频率重新计算人脉评分"},
		"connect_drift_reminder":   {Name: "人脉久未联系提醒", Description: "每天汇总久未联系的人脉"},
...
var jobMeta = map[string]jobUIMeta{
	...
	"connect_activity_ingest":  {Name: "인맥 활동 자동 수집", Description: "매일 새벽 Gmail과 Google 캘린더에서 활동을 인맥 타임라인으로 가져옵니다 (Google OAuth 필요)", Level: "external", CanDisable: true},
	"connect_score_recompute":  {Name: "인맥 점수 재계산", Description: "매일 새벽 최근 활동과 목표 빈도에 맞춰 인맥 점수를 다시 계산합니다", Level: "maintenance", CanDisable: true},
	"connect_drift_reminder":   {Name: "인맥 드리프트 알림", Description: "연락이 뜸해진 인연을 매일 아침 요약해서 알려드립니다", Level: "pattern", CanDisable: true},
