...
var translations = map[string]map[string]i18nEntry{
	"en": {
		...
		"connect-ocr":              {DisplayName: "Business Card OCR", Description: "Scan a business card image with Gemini Vision and create a new Connect entry from the extracted fields"},
		"connect-memo":             {DisplayName: "Connect Memo", Description: "Add, replace, or clear the Context Memo on a Connect entry; looks up the target by fuzzy name when the agent doesn't have the UUID yet"},
		"connect-activity":         {DisplayName: "Connect Activity", Description: "Record interactions on the Connect activity timeline, list recent entries, or sync from Gmail and Google Calendar"},
		"connect-contacts-import":  {DisplayName: "Google Contacts Import", Description: "One-shot bulk import of Google Contacts (People API) into the Connect list, with email/phone dedup against existing entries"},
	...
	"ja": {
		...
		"connect-ocr":              {DisplayName: "名刺スキャン", Description: "Gemini Visionで名刺を読み取り、人脈(Connect)に新規エントリを作成"},
		"connect-memo":             {DisplayName: "人脈メモ", Description: "人脈(Connect)エントリのContext Memoを追加・置換・削除。UUIDを知らない場合は名前で曖昧検索して対象を特定"},
		"connect-activity":         {DisplayName: "人脈アクティビティ", Description: "人脈タイムラインへのアクティビティ記録、一覧表示、GmailとGoogleカレンダーからの同期"},
		"connect-contacts-import":  {DisplayName: "Google連絡先取り込み", Description: "Googleコンタクト(People API)を人脈(Connect)に一括取り込み。既存エントリとメール/電話で重複排除"},
	...
	"zh": {
		...
		"connect-ocr":              {DisplayName: "名片扫描", Description: "使用Gemini Vision扫描名片并在人脉(Connect)中创建新条目"},
		"connect-memo":             {DisplayName: "人脉备注", Description: "为人脉(Connect)条目添加、替换或清除Context Memo;未知UUID时按姓名模糊匹配定位"},
		"connect-activity":         {DisplayName: "人脉活动", Description: "记录人脉时间轴上的活动，列出最近条目，或从Gmail和Google日历同步"},
		"connect-contacts-import":  {DisplayName: "Google通讯录导入", Description: "通过 People API 一次性批量导入 Google 通讯录到人脉，按邮箱/电话与现有条目去重"},
