...
type TelegramHandler struct {
	...
	// webhook + linking flows in this file stop embedding raw SQL.
	convUC *conversationusecase.UseCase

	// baseCtx is the server-level lifetime context. Webhook goroutines
	// derive their context from it (instead of context.Background())
	// so a SIGTERM actually cancels in-flight HandleUpdate runs
	// during the Echo shutdown window. Wired by SetBaseContext at
	// server startup; defaults to context.Background() until then.
	baseCtx context.Context
}

func NewTelegramHandler(db *database.DB, cfg *config.Config, agentClient *agentgrpc.AgentClient, convUC *conversationusecase.UseCase, logger *zap.Logger) *TelegramHandler {
	h := &TelegramHandler{
		db:          db,
		config:      cfg,
...
// SetBaseContext wires the server's lifetime context so webhook
// goroutines get cancelled when Echo starts shutting down. Called
// once at bootstrap; subsequent calls overwrite the previous value
// but there is no reason to do this at runtime.
func (h *TelegramHandler) SetBaseContext(ctx context.Context) {
	h.baseCtx = ctx
}
...
func (h *TelegramHandler) HandleUpdate(
	...
	if len(photoFileIDs) > 0 {
		...
		for _, fileID := range photoFileIDs {
			...
			if len(data) <= 4*1024*1024 {
				// Pass raw bytes through to the agent — proto3 bytes field
				// carries binary natively. Previously we base64-encoded here
				// only for the gateway's own grpc client to decode right back
				// into bytes (see grpc/client.go: ImageContent.Data was string).
			...
			if h.minio != nil {
				...
				if upErr != nil {
					...
					meta := exif.ExtractImageMetadata(data)
					...
					h.db.Pool().Exec(ctx, //nolint:errcheck
			...
			if dlErr != nil {
				...
				if upErr != nil {
					...
					if strings.HasPrefix(strings.ToLower(ct), "image/") {
						docMeta = exif.ExtractImageMetadata(data)
					}
	...
	pInfo := chatctx.ResolvePersona(ctx, h.db, info.sessionID, info.userID, h.config.EncryptionKey)
	...
	h.convUC.AppendMessage(ctx, repository.MessageInsert{
		ID:             uuid.New(),
	...
	go func() {
		defer initWg.Done()
		tgTimezone, _ := chatctx.CachedUserPrefs(ctx, h.db, info.userID)

		// Parallelise the four independent per-user lookups. resolveSkillEnv
		// returns both the env JSON and the provider list so the separate
		// integration_keys query is gone.
		var (
			tgSecondaryModel      string
			tgFallbackChain       string
			tgSkillEnvJSON        string
			tgConfiguredProviders []string
			tgDisabledSkillsJSON  string
		)
		var tgResolveWG sync.WaitGroup
		tgResolveWG.Add(4)
		go func() {
			defer tgResolveWG.Done()
			tgSecondaryModel = chatctx.ResolveAssignedModel(ctx, h.db, info.userID, "secondary")
		}()
		go func() {
			defer tgResolveWG.Done()
			tgFallbackChain = chatctx.ResolveFallbackChain(ctx, h.db, info.userID, h.config.EncryptionKey, tgProvider)
		}()
		go func() {
			defer tgResolveWG.Done()
			tgSkillEnvJSON, tgConfiguredProviders = chatctx.ResolveSkillEnv(ctx, h.db, info.userID, h.config.EncryptionKey)
		}()
		go func() {
			defer tgResolveWG.Done()
			tgDisabledSkillsJSON = chatctx.ResolveDisabledSkillsJSON(ctx, h.db, info.userID)
		}()
		tgResolveWG.Wait()

		events, grpcErr = h.agentClient.StreamChat(ctx, agentgrpc.ChatInput{
			...
			Timezone:            tgTimezone,
			SecondaryModel:      tgSecondaryModel,
			PreviousMessages:    tgRecentMsgs,
			Images:              agentImages,
			ConfiguredProviders: tgConfiguredProviders,
			Platform:            "telegram",
			FallbackProviders:   tgFallbackChain,
			SkillEnvJSON:        tgSkillEnvJSON,
			DisabledSkillsJSON:  tgDisabledSkillsJSON,
		})
	}()
	initWg.Wait()
...
loop:
	...
	if fullReply == "" && streamErrorMsg != "" {
		...
		if cat := chatctx.ClassifyAgentError(streamErrorMsg); cat != chatctx.ErrCatUnknown {
			_, lang := chatctx.CachedUserPrefs(ctx, h.db, info.userID)
	...
	if fullReply != "" {
		h.convUC.AppendMessage(ctx, repository.MessageInsert{
			...
			BotName:        tgBotName,
			ModelUsed:      tgModel,
			InputTokens:    tgInputTokens,
			OutputTokens:   tgOutputTokens,
			ContextTokens:  tgContextTokens,
			ContextWindow:  tgContextWindow,
		})
		chatctx.InsertUsageLog(ctx, h.db, info.userID, tgModel, tgProvider, tgInputTokens, tgCacheReadTokens, tgOutputTokens, "chat", 0)
	}
	h.convUC.Touch(ctx, info.sessionID)
...
func (h *TelegramHandler) Webhook(c echo.Context) error {
	// Verify X-Telegram-Bot-Api-Secret-Token before trusting the payload.
	// Telegram attaches this header to every webhook delivery when `secret_token`
	// was provided to setWebhook. Requests lacking a matching secret are rejected
	// with 401 so nobody can impersonate Telegram by POSTing to the URL directly.
	expected := h.config.TelegramWebhookSecret
	if expected == "" {
		h.logger.Error("Telegram webhook called but TELEGRAM_WEBHOOK_SECRET is not configured; rejecting")
		return c.JSON(http.StatusUnauthorized, map[string]string{"error": "webhook secret not configured"})
	}
	// Use a constant-time comparison so that network-adjacent attackers
	// cannot byte-by-byte probe the secret via response timing.
	provided := c.Request().Header.Get("X-Telegram-Bot-Api-Secret-Token")
	if subtle.ConstantTimeCompare([]byte(provided), []byte(expected)) != 1 {
		h.logger.Warn("Telegram webhook secret mismatch", zap.String("remote", c.RealIP()))
	...
	if msg.MediaGroupID != "" {
		...
		entry.timer = time.AfterFunc(1500*time.Millisecond, func() {
			...
			ct := entry.chatType
			entry.mu.Unlock()
			go h.HandleUpdate(h.baseCtx, tUID, cID, mID, fn, un, txt, ct, pFileIDs, "", dFileID)
		})
		entry.mu.Unlock()
	...
	// Use the server lifetime context (h.baseCtx) instead of the
	// per-request context so the goroutine outlives the 200-OK
	// response to Telegram. A SIGTERM cancels h.baseCtx during
	// Echo's shutdown window, giving in-flight updates a chance
	// to finish cleanly before the process exits.
	go h.HandleUpdate(
		h.baseCtx,
