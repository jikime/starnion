...
func (h *TelegramHandler) findOrCreateUserByTelegram(ctx context.Context, telegramUserID, chatID int64, firstName, username string) (*userBotInfo, error) {
	...
	if csErr := h.db.Pool().QueryRow(ctx,
	...
	// FindLatestByThread picks the most recently created conversation.
	channelKey := fmt.Sprintf("%d", chatID)
	convTitle := telegramDisplayTitle(firstName, username, telegramUserID)
	sessionID, found, lookupErr := h.convUC.FindLatestByThread(ctx, userID, "telegram", channelKey)
	if lookupErr != nil || !found {
		sessionID, err = h.convUC.CreateWithThread(ctx, userID, convTitle, "telegram", channelKey)
		if err != nil {
			return nil, fmt.Errorf("create telegram conversation: %w", err)
		}
	} else {
		// Update title in case username changed since last conversation.
		if err := h.convUC.UpdateTitle(ctx, sessionID, convTitle); err != nil {
...
func (h *TelegramHandler) LinkTelegram(c echo.Context) error {
	...
	if err == nil {
		...
		h.db.Pool().Exec(ctx, `UPDATE finance_records SET user_id = $1 WHERE user_id = $2`, userID, ghostID)
		// Migrate planner data
		h.db.Pool().Exec(ctx, `UPDATE planner_tasks SET user_id = $1 WHERE user_id = $2`, userID, ghostID)
		h.db.Pool().Exec(ctx, `UPDATE planner_roles SET user_id = $1 WHERE user_id = $2`, userID, ghostID)
		h.db.Pool().Exec(ctx, `UPDATE planner_goals SET user_id = $1 WHERE user_id = $2`, userID, ghostID)
		h.db.Pool().Exec(ctx, `UPDATE planner_diary SET user_id = $1 WHERE user_id = $2`, userID, ghostID)
		// Migrate chat_sessions (and cascade to chat_messages via FK)
		h.db.Pool().Exec(ctx, `UPDATE chat_sessions SET user_id = $1 WHERE user_id = $2`, userID, ghostID)
		// Migrate conversations (and cascade to messages via FK)
		h.db.Pool().Exec(ctx, `UPDATE conversations SET user_id = $1 WHERE user_id = $2`, userID, ghostID)
		// Delete ghost user
		h.db.Pool().Exec(ctx, `DELETE FROM users WHERE id = $1`, ghostID)
...
func (h *TelegramHandler) LinkTelegramByCode(c echo.Context) error {
	...
	for _, q := range migrateTables {
		h.db.Pool().Exec(ctx, q, userID, ghostID)
	}
	h.db.Pool().Exec(ctx, `DELETE FROM users WHERE id = $1`, ghostID)
