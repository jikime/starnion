...
func main() {
	...
	// CLI token — verifies email + password, issues a 30-day JWT for CLI use.
	// POST /auth/cli-token  Body: { "email": "...", "password": "..." }
	//                       Response: { "token": "...", "user_id": "...", "email": "...", "name": "...", "expires_at": "..." }
	e.POST("/auth/cli-token", func(c echo.Context) error {
		if db == nil {
			return c.JSON(http.StatusServiceUnavailable, map[string]string{"error": "database unavailable"})
		}
		var req struct {
		...
		if err != nil {
			...
			log.Error().Err(err).Str("email", req.Email).Msg("cli-token: login failed")
			return c.JSON(http.StatusInternalServerError, map[string]string{"error": "login failed"})
		}

		token, err := authSvc.IssueToken(user.UserID, "cli")
		if err != nil {
	...
	// POST /auth/link  Body: { "code": "NION-XXXXXX" }
	...
	if db != nil {
		...
		api.POST("/providers/custom/models", modelsHandler.FetchCustomModels)
		api.DELETE("/providers/:provider", modelsHandler.DeleteProvider)
		api.GET("/model-assignments", modelsHandler.ListModelAssignments)
		api.POST("/model-assignments", modelsHandler.UpsertModelAssignment)
