...
const MOOD_CONFIG: Record<string, { label: string; labelKo: string; color: string; emoji: string }> = {
  매우좋음: { label: "Joy",   labelKo: "매우좋음", color: "#fbbf24", emoji: "✨" },
  좋음:    { label: "Peace", labelKo: "좋음",     color: "#34d399", emoji: "🌿" },
  보통:    { label: "Calm",  labelKo: "보통",     color: "#a78bfa", emoji: "💜" },
  나쁨:    { label: "Sad",   labelKo: "나쁨",     color: "#60a5fa", emoji: "💙" },
  매우나쁨: { label: "Down",  labelKo: "매우나쁨", color: "#818cf8", emoji: "🌙" },
}

// Healing message mood key → i18n prefix mapping
const HEALING_MSG_KEYS: Record<string, string> = {
  매우좋음: "healingJoy",
  좋음:     "healingPeace",
  보통:     "healingCalm",
  나쁨:     "healingSad",
...
function MoodCalendar({ entries }: { entries: DiaryEntry[] }) {
  ...
  const CAL_DAYS = [t("calSun"), t("calMon"), t("calTue"), t("calWed"), t("calThu"), t("calFri"), t("calSat")]
    ...
    <div className="rounded-2xl p-3" style={{
      ...
      <div className="flex items-center justify-between mb-2.5">
        ...
        <span className="text-[13px] font-semibold tracking-widest"
          ...
          {t("calYearMonth", { year: viewYear, month: viewMonth + 1 })}
        </span>
        <Button
          variant="ghost"
          size="icon"
      ...
      <div className="flex flex-wrap gap-x-2 gap-y-0.5 mt-2.5 pt-2"
        ...
        {Object.entries(MOOD_CONFIG).map(([, cfg]) => (
          <div key={cfg.labelKo} className="flex items-center gap-1">
            <div style={{ width: 5, height: 5, borderRadius: "50%", background: cfg.color,
              boxShadow: `0 0 3px ${cfg.color}88` }} />
            <span style={{ fontSize: "10px", color: "rgba(255,255,255,0.65)" }}>{cfg.emoji} {t(`mood_${cfg.label.toLowerCase()}`)}</span>
          </div>
        ))}
      </div>
    </div>
...
function HealingTree({ emotions, totalDiaries, entries, streak }: {
  emotions: EmotionStat[]
  totalDiaries: number
  entries: DiaryEntry[]
  streak: number
  ...
  return (
    <div className="flex flex-col gap-3 h-full">
      ...
      <div className="flex-1 rounded-2xl p-3" style={{
        ...
        <div className="mt-2.5 pt-2 flex items-center justify-between text-[12px]" style={{
          borderTop: "1px solid rgba(255,255,255,0.06)",
          color: "rgba(255,255,255,0.62)",
        }}>
          <span>{t("totalMemories", { n: totalDiaries })}</span>
          {streak > 0 && (
...
function NionCenter({
  recentMood, nionState, activeMood, savingMood, moodSaved, onMoodSelect,
}: {
  recentMood: string
  nionState: string
  activeMood: string
  ...
  // 메시지 다양성: 감정 변경 시 랜덤 인덱스 선택
  const msgIndex = useMemo(() => {
    return Math.floor(Math.random() * HEALING_MSG_COUNT)
  }, [displayMood]) // eslint-disable-line react-hooks/exhaustive-deps
  const healingMsgKey = `${HEALING_MSG_KEYS[displayMood] ?? HEALING_MSG_KEYS["보통"]}_${msgIndex}`
  const healingMsg = t(healingMsgKey)
    ...
    <div className="flex flex-col items-center justify-center gap-4 h-full">
      ...
      <div className="text-center space-y-1">
        ...
        <div className="flex items-center gap-2 justify-center">
          ...
          <span className="text-base font-medium" style={{ color: moodCfg.color }}>{t(`mood_${moodCfg.label.toLowerCase()}`)}</span>
        </div>
      </div>
      ...
      <div className="flex flex-col items-center gap-1.5">
        ...
        <div className="text-[11px] tracking-widest uppercase flex items-center gap-1.5" style={{ color: "rgba(255,255,255,0.52)" }}>
          {savingMood ? (
            <><Loader2 className="size-2.5 animate-spin" /> {t("saving")}</>
          ) : moodSaved ? (
            <span style={{ color: "#34d399" }}>✓ {t("saved")}</span>
          ) : (
            t("selectTodayMood")
          )}
        </div>
    ...
    <div className="flex flex-col gap-3 h-full">
      ...
      <div className="rounded-2xl p-3" style={{
        ...
        <div className="flex items-center justify-center gap-4">
          ...
          <div className="flex flex-col items-center gap-1">
            ...
            <img src="/wellness-coffee.png" alt={t("coffee")} width={52} height={52}
          ...
          <div className="flex flex-col items-center gap-1">
            {/* eslint-disable-next-line @next/next/no-img-element */}
            <img src="/wellness-chair.png" alt={t("chair")} width={72} height={72}
          ...
          <div className="flex flex-col items-center gap-1">
            ...
            <div className="text-[11px] text-center" style={{ color: "rgba(255,255,255,0.62)" }}>{t("thisWeekRecord")}</div>
          </div>
        </div>
      </div>
      ...
      <div className="rounded-2xl p-3" style={{
        ...
        <div className="flex justify-around">
          ...
          <div className="flex flex-col items-center gap-1.5 opacity-50 cursor-not-allowed select-none">
            <div
              className="w-14 h-14 rounded-full flex items-center justify-center"
              style={{
                background: "radial-gradient(circle, #34d39922 0%, #34d39908 100%)",
                border: "1.5px dashed #34d39944",
              }}
            >
              <span className="text-xl">🧘</span>
            </div>
            <div className="text-center">
              <span className="text-[12px] block" style={{ color: "rgba(255,255,255,0.55)" }}>{t("meditation")}</span>
              <span className="text-[10px]" style={{ color: "#34d39988" }}>{t("comingSoon")}</span>
            </div>
          </div>
          <MindFlower label={t("diary")} href="/diary" color="#a78bfa">
            <BookOpen size={20} />
          </MindFlower>
          <MindFlower label={t("chat")} href="/chat" color="#f472b6">
      ...
      <div className="flex-1 rounded-2xl p-3" style={{
        ...
        {goals.length === 0 ? (
          <div className="flex flex-col items-center gap-2 py-3 text-center">
            ...
            <Link href="/goals"
              className="text-[11px] px-3 py-1 rounded-full transition-opacity hover:opacity-70"
              style={{
          ...
          <div className="space-y-2.5">
            ...
            {goals.length > 3 && (
              <Link href="/goals" className="block text-center text-[11px] mt-1 hover:opacity-70 transition-opacity"
                style={{ color: "rgba(167,139,250,0.6)" }}>
                {t("moreGoals", { n: goals.length - 3 })}
              </Link>
            )}
          </div>
        )}
...
export default function WellnessPage() {
  ...
  const handleMoodSelect = useCallback(async (mood: string) => {
    ...
    try {
      ...
      await fetch("/api/diary", {
        ...
        body: JSON.stringify({ mood, content: "", entry_date: today }),
      })
      setMoodSaved(true)
      setTimeout(() => setMoodSaved(false), 2500)
    } catch {
      // 저장 실패 시 silent - UI 상태만 업데이트 됨
    } finally {
      setSavingMood(false)
    }
  }, [])
  ...
  return (
    ...
    >
      ...
      {/* Main 3-column grid */}
      {loading ? (
        <div className="relative flex-1 grid gap-4 px-5 py-3 grid-cols-1 md:grid-cols-[1fr_1.1fr_1fr]" style={{ zIndex: 10 }}>
          {/* Left column */}
          ...
          <div className="flex flex-col items-center gap-4">
            <Skeleton className="size-48 rounded-full" style={{ background: "rgba(251,191,36,0.12)" }} />
            <Skeleton className="h-5 w-36" />
            <div className="w-full rounded-2xl border border-border bg-card/60 p-4 space-y-2">
            ...
            <div className="rounded-2xl border border-border bg-card/60 p-4 space-y-2">
              ...
              {Array.from({ length: 4 }).map((_, i) => (
                <div key={i} className="flex items-center gap-2">
                  <Skeleton className="size-5 rounded-full shrink-0" style={{ background: "rgba(251,191,36,0.15)" }} />
        ...
        <div
          className="relative flex-1 grid gap-4 px-5 py-3 grid-cols-1 md:grid-cols-[1fr_1.1fr_1fr]"
          style={{ zIndex: 10, animation: "ws-rise 0.4s ease-out" }}
        >
          {/* Left: Healing Tree + Emotion chart + Calendar */}
          <HealingTree emotions={data.emotions} totalDiaries={data.totalDiaries} entries={data.entries} streak={data.streak} />
