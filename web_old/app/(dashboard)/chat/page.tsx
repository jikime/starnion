...
import { type Conversation, type Persona, groupByDate } from "./_components/types"
...
function ChatPageInner() {
  ...
  // Default closed on mobile, open on desktop — set after mount to avoid hydration mismatch
  useEffect(() => {
    const isDesktop = window.matchMedia("(min-width: 768px)").matches
    setSidebarOpen(isDesktop)
  }, [])

  // Model override (independent of persona's model)
  const [selectedModelId, setSelectedModelId] = useState<string>(DEFAULT_MODEL_ID)

  // Available bot personas from settings
  ...
  // Load bot personas from settings
  useEffect(() => {
  ...
  // Called by the WS hook when gateway auto-creates a new conversation on first send.
  ...
  // Assign a bot soul (persona) to the active conversation
