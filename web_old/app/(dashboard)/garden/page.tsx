...
interface GardenData {
  ...
  webSearchEnabled: boolean // 정찰용 별 활성 여부
  hasLoadError: boolean    // 일부 API 실패 여부
}
      ...
      >
        <div className="p-4 space-y-2.5">
          <div className="flex items-center gap-2">
            ...
            <span className="text-sm font-bold tracking-wide text-popover-foreground">{title}</span>
          </div>
  ...
  return (
    <div className="absolute" style={{ right: "4%", top: "38%", zIndex: 8 }}>
      <GardenPopover
        ...
        align="center"
        stats={[
          { label: t("ddaySchedule"), value: t("countUnit", { n: allUpcoming.length }), color: capColor },
          ...allUpcoming.slice(0, 4).map(d => ({
            label: d.title.length > 7 ? d.title.slice(0, 7) + "…" : d.title,
            value: d.dday_value === 0 ? "D-Day!" : `D-${d.dday_value}`,
            color: d.dday_value <= 3 ? "#f87171" : d.dday_value <= 7 ? "#fbbf24" : "#a78bfa",
          })),
          ...(allUpcoming.length > 4 ? [{ label: t("more", { n: allUpcoming.length - 4 }), value: t("moreExist"), color: `${capColor}88` }] : []),
        ]}
        description={t("timeCapsuleDesc")}
      >
    ...
    >
      ...
      {isWithering && (
        <div className="absolute left-1/2 -translate-x-1/2 -bottom-6 whitespace-nowrap text-xs text-center"
          ...
          {isSevere ? t("treeSevere") : t("treeWithering")}
        </div>
      )}
    </div>
  )
...
function SpendingCloud({ isRaining, categories, accent }: {
  ...
  return (
    <div className="absolute" style={{ right: "6%", top: "11%", zIndex: 8 }}>
      <GardenPopover
        icon="☁️"
        title={t("spendingCloud")}
        accent={isRaining ? "#f87171" : accent}
        side="bottom"
        align="end"
        stats={[
          { label: t("status"), value: isRaining ? t("overBudget") : t("budgetOk"), color: isRaining ? "#f87171" : "#34d399" },
          ...(categories.length > 0
            ? [{ label: t("overCategories"), value: t("countUnit", { n: categories.length }), color: "#fca5a5" }]
            : []),
          ...categories.slice(0, 3).map(c => ({ label: "  •", value: c, color: "#fca5a5" })),
        ]}
        description={isRaining ? t("spendingCloudDescRaining") : t("spendingCloudDescOk")}
      >
...
function GoalFlowers({ goals, accent }: { goals: Goal[]; accent: string }) {
  ...
  const hiddenCount = Math.max(0, goals.length - 4)
  ...
  return (
    <div className="absolute" style={{ left: "2%", bottom: "9%", zIndex: 9 }}>
      <GardenPopover
        ...
        align="start"
        stats={[
          { label: t("totalGoals"), value: t("countUnit", { n: goals.length }) },
          ...displayed.map(g => ({
          ...
          ...(hiddenCount > 0 ? [{ label: t("moreGoals", { n: hiddenCount }), value: t("checkGoalMgr"), color: `${accent}99` }] : []),
        ]}
        description={t("goalFlowerDesc")}
      >
        <div className="cursor-pointer hover:brightness-110 transition-all">
          <svg viewBox={`-12 -22 ${w} 180`} width={w} height={180} style={{ overflow: "visible" }}>
            <text x={(w - 24) / 2} y={-8} textAnchor="middle" fontSize="11"
              fill="white" opacity="0.55" letterSpacing="2">{t("goalFlower")}</text>
          ...
          {hiddenCount > 0 && (
            <div className="text-center -mt-1">
              <span style={{ fontSize: 10, color: `${accent}99` }}>+{hiddenCount} {t("more")}</span>
            </div>
          )}
        </div>
      </GardenPopover>
...
const MOOD_LABEL_KEYS: Record<string, string> = {
  매우좋음: "moodVeryGood", 좋음: "moodGood", 보통: "moodNormal", 나쁨: "moodBad", 매우나쁨: "moodVeryBad",
}
...
function EmotionSeeds({ diaryMoods, accent }: { diaryMoods: string[]; accent: string }) {
  ...
  return (
    <div className="absolute" style={{ bottom: "14%", right: "9%", zIndex: 7 }}>
      <GardenPopover
        icon="🌱"
        title={t("emotionSeeds")}
        accent={accent}
        side="top"
        align="end"
        stats={[
          { label: t("recordCount"), value: t("countUnit", { n: seeds.length }) },
          ...Object.entries(moodCounts).map(([mood, cnt]) => ({
            label: MOOD_LABEL_KEYS[mood] ? t(MOOD_LABEL_KEYS[mood]) : mood,
            value: t("dayUnit", { n: cnt }),
            color: SEED_COLORS[mood] ?? accent,
          })),
        ]}
        description={t("emotionSeedsDesc")}
      >
        <div className="cursor-pointer hover:brightness-110 transition-all">
  ...
  return (
    <div className="absolute" style={{ left: "24%", bottom: "9%", zIndex: 6 }}>
      <GardenPopover
        ...
        align="center"
        stats={[
          { label: t("savedImages"), value: t("imageUnit", { n: imageCount }), color: "#93c5fd" },
          { label: t("surfaceReflections"), value: t("countUnit", { n: reflections }), color: "#60a5fa" },
        ]}
        description={t("magicLakeDesc")}
      >
        <div className="cursor-pointer hover:scale-105 transition-transform">
  ...
  return (
    <div className="absolute" style={{ left: "5%", top: "16%", zIndex: 8 }}>
      <GardenPopover
        ...
        align="start"
        stats={[
          { label: t("audioFiles"), value: t("countUnit", { n: audioCount }), color: "#c4b5fd" },
        ]}
        description={t("windBellDesc")}
      >
        <div
        ...
        >
          ...
          <span className="text-xs font-medium" style={{ color: "#a78bfacc" }}>{t("countUnit", { n: audioCount })}</span>
        </div>
      </GardenPopover>
    </div>
  )
...
function KnowledgeBackpack({ docCount, accent: _accent }: { docCount: number; accent: string }) {
  ...
  return (
    <div className="absolute" style={{ left: "5%", top: "44%", zIndex: 8 }}>
      <GardenPopover
        icon="🎒"
        title={t("knowledgeBackpack")}
        accent="#fbbf24"
        side="right"
        align="center"
        stats={[
          { label: t("savedDocs"), value: t("countUnit", { n: docCount }), color: "#fde68a" },
        ]}
        description={t("knowledgeBackpackDesc")}
      >
        <div
          className="flex flex-col items-center gap-1 cursor-pointer hover:scale-110 transition-transform"
        ...
        >
          ...
          <span className="text-xs font-medium" style={{ color: "#fbbf24bb" }}>{t("knowledgeBackpack")}</span>
        </div>
      </GardenPopover>
    </div>
  )
  ...
  return (
    <div className="absolute" style={{ right: "22%", bottom: "24%", zIndex: 8 }}>
      <GardenPopover
        ...
        align="center"
        stats={[
          { label: t("activeSkills"), value: t("countUnit", { n: skillCount }), color: "#e879f9" },
          { label: t("gemsEquipped"), value: t("countUnit", { n: gems }), color: "#c084fc" },
        ]}
        description={t("magicWandDesc")}
      >
        <div
          className="flex flex-col items-center gap-1 cursor-pointer hover:scale-110 transition-transform"
        ...
        >
          ...
          <span className="text-xs font-medium" style={{ color: "rgba(255,255,255,0.55)" }}>{t("magicWand")}</span>
        </div>
      </GardenPopover>
    </div>
  )
...
function EnergyCharger({ totalTokens, accent: _accent }: { totalTokens: number; accent: string }) {
  ...
  const fillColor = pct >= 80 ? "#f87171"
    ...
    : "#34d399"
  const isHot = pct >= 60
  const statusLabel = pct >= 80 ? `🔴 ${t("statusOverload")}` : pct >= 60 ? `🟠 ${t("statusHigh")}` : pct >= 40 ? `🟡 ${t("statusMedium")}` : `🟢 ${t("statusFree")}`
  return (
    <div className="absolute" style={{ right: "4%", bottom: "8%", zIndex: 8 }}>
      <GardenPopover
        icon="🔋"
        title={t("energyCharger")}
        accent={fillColor}
        side="top"
        align="end"
        stats={[
          { label: t("usage"), value: `${pct.toFixed(1)}%`, color: fillColor },
          { label: t("cumulativeTokens"), value: formatTokens(totalTokens), color: fillColor },
          { label: t("monthlyLimit"), value: t("tenMillionTokens") },
          { label: t("status"), value: statusLabel },
        ]}
        description={t("energyChargerDesc")}
      >
        <div className="flex flex-col items-center gap-1 cursor-pointer hover:scale-110 transition-transform">
          ...
          <span className="text-xs font-medium" style={{ color: `${fillColor}cc` }}>{t("energy")}</span>
        </div>
      </GardenPopover>
    </div>
  )
...
function ScoutStar({ accent: _accent, webSearchEnabled }: { accent: string; webSearchEnabled: boolean }) {
  const t = useTranslations("garden")
  const statusLabel = webSearchEnabled ? `🟢 ${t("active")}` : `⚫ ${t("inactive")}`
  const statusColor = webSearchEnabled ? "#34d399" : "#6b7280"
  return (
    <div className="absolute" style={{ left: "46%", top: "6%", zIndex: 7 }}>
      <GardenPopover
        ...
        description={webSearchEnabled ? t("scoutStarDescActive") : t("scoutStarDescInactive")}
      >
        <div className="cursor-pointer hover:scale-110 transition-transform">
          ...
          <div className="text-center mt-1">
            <span className="text-[11px] font-medium" style={{ color: "#fde68a88" }}>{t("scoutStar")}</span>
          </div>
        </div>
      </GardenPopover>
    </div>
    ...
    <div className="absolute bottom-0 left-0 right-0" style={{ zIndex: 4 }}>
      <svg viewBox="0 0 1200 32" preserveAspectRatio="none" width="100%" height="32">
        ...
        <text x="600" y="12" textAnchor="middle" fontSize="8" fill="white" opacity={0.30 * brightness} letterSpacing="3">
          {t("galaxyPath")} · {t("integratedCount", { n: integrationCount })}
        </text>
      </svg>
      ...
      <div className="absolute right-4 bottom-1" style={{ zIndex: 10 }}>
        <GardenPopover
          ...
          align="end"
          stats={[
            { label: t("integratedServices"), value: t("countUnit", { n: integrationCount }), color: "#818cf8" },
            { label: t("galaxyBrightness"), value: `${Math.round(brightness * 100)}%`, color: "#a5b4fc" },
          ]}
          description={t("galaxyPathDesc")}
        >
          <button
          ...
          >
            🌌 {t("integratedCount", { n: integrationCount })}
          </button>
        </GardenPopover>
      </div>
    </div>
      ...
      <div className="relative rounded-2xl p-6 shadow-2xl"
        ...
        <div className="text-center mb-3">
          <div className="text-2xl font-bold" style={{ color: isOver ? "#f87171" : accent }}>
            {isOver ? "+" : ""}{shortKRW(Math.abs(budgetRemaining))}{t("wonSuffix")}
          </div>
          <div className="text-xs mt-0.5" style={{ color: isOver ? "#fca5a5" : "#c4b5fd" }}>
            {isOver ? t("budgetOver") : t("budgetRemaining")}
          </div>
        </div>
        ...
        <div className="space-y-2 text-sm">
          <div className="flex justify-between items-center">
            <span className="text-white/55">{t("totalBudget")}</span>
            <span className="font-semibold text-white">{shortKRW(totalBudget)}{t("wonSuffix")}</span>
          </div>
          <div className="flex justify-between items-center">
            <span className="text-white/55">{t("totalSpent")}</span>
            <span className={`font-semibold ${isOver ? "text-red-400" : "text-white"}`}>
              {shortKRW(totalSpent)}{t("wonSuffix")}
            </span>
          </div>
          ...
          <div className="flex justify-between text-xs">
            <span className="text-white/38">{t("spendRate", { n: pct.toFixed(1) })}</span>
            <span style={{ color: isOver ? "#f87171" : accent }}>{t("saveRate", { n: savingsRate.toFixed(1) })}</span>
          </div>
        </div>

        <div className="mt-3 rounded-xl p-2.5 text-xs text-center"
          style={{ background: `${accent}14`, color: `${accent}cc` }}>
          {savingsRate >= 60 ? t("budgetMsg60")
            : savingsRate >= 30 ? t("budgetMsg30")
            : isOver ? t("budgetMsgOver")
            : t("budgetMsgLow")}
        </div>
        ...
        <Link
          href="/budget"
          onClick={onClose}
          className="mt-2 block text-center text-xs rounded-xl py-2 transition-opacity hover:opacity-70"
          style={{
...
export default function GardenPage() {
  ...
  const load = useCallback(async () => {
    ...
    const [budgetRes, goalsRes, diaryRes, summaryRes, ddayRes,
      ...
      await Promise.allSettled([
        ...
        fetch("/api/dday"),
        // Phase 2
        fetch("/api/files?type=image&limit=50"),
        fetch("/api/files?type=audio&limit=50"),
    ...
    const imagesRaw   = await json<{ files?: unknown[] }>(imagesRes)
    const audiosRaw   = await json<{ files?: unknown[] }>(audiosRes)
    const docsRaw     = await json<{ files?: unknown[] }>(docsRes)
    ...
    // Phase 2 data
    const imageList = imagesRaw?.files ?? []
    const audioList = audiosRaw?.files ?? []
    const docList   = docsRaw?.files   ?? []
    ...
    // 웹 검색 활성 여부: 전용 provider(tavily/perplexity/web_search) 또는 연동 서비스 존재 여부로 판단
    const webSearchProvider = providerList.find(p =>
      p.provider === "tavily" || p.provider === "perplexity" || p.provider === "web_search"
    )
    const webSearchEnabled = webSearchProvider
      ? (webSearchProvider.provider === "custom" ? !!webSearchProvider.baseUrl : !!webSearchProvider.hasKey)
      : integrationCount > 0

    // 핵심 API 중 실패한 것이 있으면 hasLoadError = true
    const hasLoadError = [budgetRes, goalsRes, summaryRes].some(r => r.status === "rejected" || (r.status === "fulfilled" && !r.value.ok))
    ...
    setData({
      ...
      goals,  // 전체 goals 전달 (GoalFlowers에서 slice 처리)
  ...
  // 신규 유저 온보딩: 데이터 로드 완료 후 모든 필드가 비어있으면 true
  const isNewUser = !loading && data !== null &&
    data.goals.length === 0 &&
    data.income === 0 &&
    data.diaryMoods.length === 0 &&
    data.docCount === 0 &&
    data.imageCount === 0 &&
    data.audioCount === 0
  ...
  return (
    <div className="relative w-full overflow-hidden select-none"
      ...
      {/* 정찰용 별 */}
      <ScoutStar accent={palette.accent} webSearchEnabled={data?.webSearchEnabled ?? false} />
      ...
      {loading && (
        <div className="absolute inset-0 flex items-center justify-center" style={{ zIndex: 60 }}>
          <div className="flex flex-col items-center gap-3">
            ...
            <span className="text-sm" style={{ color: `${palette.accent}88` }}>{t("loading")}</span>
          </div>
        </div>
      )}
      ...
      {isNewUser && (
        <div className="absolute inset-0 flex items-center justify-center pointer-events-none" style={{ zIndex: 30 }}>
          <div className="rounded-2xl px-8 py-7 text-center max-w-xs"
            style={{
              background: "rgba(5,8,30,0.82)",
              border: `1px solid ${palette.accent}30`,
              backdropFilter: "blur(16px)",
              animation: "gdn-rise 0.5s ease-out",
            }}>
            <div className="text-3xl mb-3">🌱</div>
            <div className="text-sm font-bold mb-2" style={{ color: palette.accent }}>
              {t("onboardingTitle")}
            </div>
            <p className="text-xs leading-relaxed mb-4" style={{ color: "rgba(255,255,255,0.5)" }}>
              {t("onboardingDesc")}
            </p>
            <div className="flex flex-col gap-1.5 text-[11px]" style={{ color: "rgba(255,255,255,0.35)" }}>
              <span>{t("onboardingHint1")}</span>
              <span>{t("onboardingHint2")}</span>
              <span>{t("onboardingHint3")}</span>
            </div>
          </div>
        </div>
      )}
      ...
      {/* API 오류 배너 */}
      {data?.hasLoadError && !loading && (
        <div className="absolute top-14 left-1/2 -translate-x-1/2 rounded-full px-4 py-1.5 text-xs"
          style={{
            zIndex: 25,
      ...
      <div className="absolute top-4 right-4 flex items-center gap-2" style={{ zIndex: 20 }}>
        {data && (
          <div className="rounded-full px-3 py-1.5 text-xs font-medium"
            ...
            {data.mood} · {data.savingsRate.toFixed(0)}% {t("savingsSuffix")}
          </div>
        )}
      ...
      {data && !showPopup && (
        <div className="absolute bottom-3 left-1/2 -translate-x-1/2 flex items-center gap-3 text-xs flex-wrap justify-center"
          ...
          {data.goals.length > 0 && <span>🌸 {t("legendGoalFlower")}</span>}
          <span className="cursor-pointer hover:opacity-70 transition-opacity font-medium"
            style={{ color: `${palette.accent}aa` }}
            onClick={() => setShowPopup(true)}>
            ✦ {t("legendAssetTree")}
          </span>
          {data.isRaining && <span>☁️ {t("legendSpendingCloud")}</span>}
          {data.meteorCount > 0 && <span>☄️ {t("legendMeteor")}</span>}
          {data.diaryMoods.length > 0 && <span>🌱 {t("legendEmotionSeed")}</span>}
          {data.ddays.filter(d => d.dday_value >= 0).length > 0 && <span>💊 {t("legendTimeCapsule")}</span>}
          {data.imageCount > 0 && <span>🌊 {t("legendMagicLake")}</span>}
          {data.audioCount > 0 && <span>🔔 {t("legendWindBell")}</span>}
          {data.docCount > 0 && <span>🎒 {t("legendKnowledgeBackpack")}</span>}
          {data.skillCount > 0 && <span>🪄 {t("legendMagicWand")}</span>}
          {data.integrationCount > 0 && <span>🌌 {t("legendGalaxyPath")}</span>}
          <span>⭐ {t("legendScoutStar")}</span>
          <span>⚡ {t("legendEnergy")}</span>
        </div>
      )}
    </div>
  )
